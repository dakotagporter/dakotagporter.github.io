...
Assuming there would only be one average price we could predict for any given car and it's features, an average error of about $12,500 above or below the actual MSRP price took place in this data. Now I'm not confident about this, but I don't think I'd be happy going to buy a car and find out that I'd possibly pay $12,000 more than what I thought. This is where the magic of machine learning models come into play. I deployed three different models in hopes that one would predict better than the rest, and that's exactly what happened. A gradient boosting regression model outperformed the linear and xgboost regression models. After splitting the data into training and test sets (a usual ML task), I noticed the model was hauntingly accurate predicting the training set. That's not always the best information becuase it shows that your model has tried too hard to exactly predict the true target value and, thus, will **not** predict well to any new data.
